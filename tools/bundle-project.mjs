...
const EXCLUDE_DIRS = [
  ...
  ['ops/refs/',      'كل ما ليس منتَجًا: مراجع تصميم (نماذج HTML/React من أدوات AI) وصور الهوية وباكند Apps Script الميّت — لا يعتمد عليها الكود'],
  ['.claude/',       'إعدادات أداة التطوير المحلّية، لا علاقة لها بالمنتج'],
  ['app/android/.idea/', 'إعدادات Android Studio المحلّية'],
  ['app/android/gradle/','ملفّات wrapper مولَّدة'],
];
...
const ORDER = [
  ...
  [/^backend\//,                      45],
  [/^ops\/docs\/plans\//,             50],
  [/^ops\/docs\/design-system\//,     51],
  [/^ops\/docs\/legal\//,             52],
  [/^ops\/docs\/history\.md$/,        53],
  [/^ops\/docs\//,                    54],
  [/^app\/android\//,                 60],
  [/^\.github\//,                     65],
];
...
head.push('- **الأمان في RLS** لا في الإخفاء: المفتاح `anon` عامّ بالتصميم، والسياسات في `db/migration/`.');
...
head.push('> وهو أهمّ ملفّ في هذا البندل بعد الكود نفسه. والسجلّ التاريخي في `ops/docs/history.md`.');
head.push('');
/* ══════════════════════════════════════════════════════════════════════════
   ...
   `db/migration/` مُشغَّلٌ فعلًا على القاعدة الحيّة. الثاني وقع فعلًا في هذا
...
head.push('              ├─ db/migration/*.sql   ← المخطّط والسياسات، تُشغَّل بيد المالك');
head.push('              └─ db/supabase/functions/ai ← دالّة حافّة (لوحات المالك الثلاث)');
head.push('```');
head.push('');
head.push('- **لا خادم تطبيقات ولا ORM**: التطبيق ينادي PostgREST مباشرةً بتوكن المستخدم،');
head.push('  فكلّ ضمانة أمنٍ في المشروع هي **سياسة RLS** في `db/migration/` لا فحصٌ في JS.');
...
head.push('');
head.push('> ⚠️ **اقرأ هذا قبل أن تقترح إصلاحًا.** وجودُ ملفٍّ في `db/migration/` **لا يعني**');
head.push('> أنّه مُشغَّل على القاعدة، ووجودُ كودٍ في `db/supabase/functions/` **لا يعني** أنّه');
head.push('> منشور. آخر قياسٍ حيّ بسؤال القاعدة: **2026-08-14**.');
head.push('');
head.push('| | الحالة |');
head.push('|---|---|');
head.push('| `db/migration/08` … `31` | ✅ **كلّها مُشغَّلة** على القاعدة الحيّة (مؤكَّدة بسؤالها لا بقراءة الملفّات) |');
head.push('| `db/supabase/functions/ai` | ✅ **منشورة وتعمل**، ومعها `GEMINI_API_KEY` و`AI_MODEL=gemini-flash-latest` في الأسرار |');
head.push('| `db/supabase/functions/push` | ✅ **منشورة، وإشعارات الدفع تعمل من طرف إلى طرف** — صفٌّ حقيقي أطلق المُشغِّل فوصل الدالّةَ فردّت `200 {sent:false,"reason":"no_token"}`، وهي الإجابة الصحيحة قبل أن يسجّل جهازٌ رمزًا |');
...
head.push('- **بيانات العملاء** (`CSV/` · `db/migration/02` · `db/migration/03`) — مستثناة بـ`.gitignore` أصلًا لأنّ المستودع عامّ.');
head.push('');
