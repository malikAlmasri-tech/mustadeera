...
function showPage(name, opts){
  ...
  /* ⚠️ وتنقّلٌ يعني كيبوردًا مغلقًا: أندرويد يُغلقه مع تغيّر الشاشة ولا يضمن
     حدثًا لـ`visualViewport` ⇒ نُصفّر هنا كذلك، فلا يبقى الشريط السفلي مخفيًّا. */
  kbClear();
  manageAutoRefresh();
...
/* 🔴 **و`kb-open` مشروطٌ بوجود حقلٍ مركَّز — لا بالحساب وحده.**
   الصنف يُنزل الشريط السفلي خارج الشاشة (`translateY(120%)`)، فبقاؤه مرفوعًا
   خطأً يعني **شريط تنقّلٍ يختفي ولا يعود** طوال الجلسة: عرَضٌ لا يوصله المستخدم
   بالكيبورد أصلًا، فيقرؤه عطلًا في التطبيق (بلاغ المالك 2026-09-08).
   والحساب وحده هشّ: `innerHeight - vv.height` يعتمد على وضع
   `windowSoftInputMode` في أندرويد، وحدثُ `resize` قد يفوت (تبويبٌ في الخلفية،
   أو إغلاقُ الكيبورد مع تنقّلٍ في اللحظة نفسها) فيبقى الصنف بلا من يُنزله.
   ⚠️ **والشرط الثاني لا يُضعف الأوّل**: كيبورد الهاتف لا يُفتَح إلّا على حقلٍ
      مركَّز، فاجتماعُهما هو التعريف الصحيح لا تشديدٌ زائد.
   ⚠️ ويُصفَّر عند فقد التركيز وعند كل تنقّل — ثلاثة مخارج لحالةٍ واحدة، لأنّ
      ثمن بقائها مرفوعةً أكبر من ثمن إنزالها مرّةً بلا داعٍ. */
function kbEditing(){
  const el = document.activeElement;
  return !!el && el !== document.body &&
         (el.matches('input:not([type=button]):not([type=submit]), textarea, [contenteditable=""], [contenteditable="true"]'));
}
function kbClear(){
...
if(window.visualViewport){
  ...
  /* الحقل يفقد التركيز ⇒ لا كيبورد. والتأجيل فريمًا لأنّ الانتقال بين حقلين
     يمرّ بلحظةٍ لا تركيز فيها، فتصفيرٌ فوريّ يُومض الشريط بينهما. */
  document.addEventListener('focusout', ()=>{ setTimeout(()=>{ if(!kbEditing()) kbClear(); }, 60); }, true);
}
